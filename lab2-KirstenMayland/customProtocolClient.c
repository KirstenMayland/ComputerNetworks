...
void worker_func( int sockfd, char statecode[2], uint8_t opcode )
{
    ...
    req.opcode = opcode;
    req.statecode[0] = statecode[0]; //TODO: check if htonl or htons
    req.statecode[1] = statecode[1]; //TODO: check if htonl or htons
    req.version = 1;
    ...
    if( res->status != RESULT_STATUS_OK ){
        ...
        len = ntohl(res->len);
        if( len > 0 ){
            fwrite( buff + sizeof(res->version) + sizeof(res->status) + sizeof(res->len), len, 1, stdout ); // number of bytes of offset sizeof(unit8) + sizeof(uint8) + sizeof(uint32) , i.e., 6.
        }
        printf("\n");          
    }
}
